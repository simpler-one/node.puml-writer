import { Statechart, StatechartItem, MetaStartStateName } from '@working-sloth/statechart-interface';
import { PumlWriterOptions, LeftToRightOption } from './interface';
...
export class PumlWriter {
    ...
    private setStates(states: StatechartItem[], transitions: Transitions): void {
        const start = states.find(state => state.name === MetaStartStateName);
        const normalStates = states.filter(state => state.name !== MetaStartStateName);
